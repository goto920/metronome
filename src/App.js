...
/// //////////// UI menus
  ...
  customPlay (event) {
    ...
    if (event.target.name === 'stop') {
      for (let notes = 0; notes < this.tickEvents.length; notes++) { this.tickEvents[notes].clear() }
...
// https://github.com/sebpiq/WAAClock/blob/master/demos/beatSequence.js
  nextTick (noteIndex) {
    const noteInterval = 60.0 / (this.state.bpm * this.params.denominator / 4)
    ...
    var currentBar = Math.floor(relativeTime / barInterval)
    ...
    return this.params.startTime + offset +
           currentBar * barInterval + noteIndex * noteInterval
  }
